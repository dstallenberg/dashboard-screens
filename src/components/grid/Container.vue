...
export default {
  ...
  methods: {
    ...
    enableResizing (box) {
      ...
      box.$on('resizeEnd', evt => {
        ...
        this.resizing.boxLayout = utils.updateBoxPosition(this.resizing.boxLayout, {
          w: this.placeholder.position.w > box.minWidth ? this.placeholder.position.w : box.minWidth, // CHANGED
          h: this.placeholder.position.h > box.minHeight ? this.placeholder.position.h : box.minHeight
        })
    ...
    createBoxLayout (...boxes) {
      boxes = boxes.filter(box => !this.getBoxLayoutById(box.id))
      ...
      if (boxes.length) {
        ...
        boxes.forEach(box => {
          let size = {
            w: box.w ? box.w : this.defaultSize.w,
            h: box.h ? box.h : this.defaultSize.h
          }
  ...
  mounted () {
    ...
    for (let i = 0; i < this.$children.length; i++) {
      boxes.push({
        id: this.$children[i].$props.boxId,
        w: this.$children[i].$props.width,
        h: this.$children[i].$props.height
      })
    }
    this.createBoxLayout(...boxes)
  },
  beforeDestroy () {
    List.delete(this)
  }
